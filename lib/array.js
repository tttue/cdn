 ...
 * getChildValue('user.id', cars[0]) = 1
 * getChildValue('user.id', cars[1]) = 5
 *
 * @param {string} name
 * @param {object} orgItem
 * @returns
 */
function getChildValue(name, orgItem) {
...
Array.prototype.sortByProperty = function (name, bIgnoreCase = true, bReversed = false) {
  ...
  // })
  let bCheckProperty = this.every(function (elem) {
    return getChildValue(name, elem) !== undefined
  })
  if (!bCheckProperty) return undefined
  return this.sort(function (a, b) {
    let aValue = getChildValue(name, a)
    let bValue = getChildValue(name, b)
